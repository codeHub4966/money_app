...
class _InsightsTabState extends ConsumerState<InsightsTab> {
    ...
    return Center(
      child: Padding(
        ...
        child: Text(
          ...
          style:
              const TextStyle(fontSize: 14, color: AppTheme.onSurfaceVariant),
        ),
      ),
    );
  }
  ...
  Widget _buildForecastCard(_MonthData data) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
              color: const Color(0xFF0A1E4F).withOpacity(0.3),
              blurRadius: 32,
            ...
            decoration: BoxDecoration(
              color: fc.flat
                  ...
                  : (fc.momPct! > 0
                      ...
                      : const Color(0xFFD6F3EA)),
              borderRadius: BorderRadius.circular(10),
            ),
            child: Text(
              fc.flat
                  ? 'Flat vs ${rm(fc.prevMonthTotal!)} last month'
                  : '${fc.momPct!.abs().toStringAsFixed(0)}% ${fc.momPct! > 0 ? 'higher' : 'lower'} than last month',
              style: TextStyle(
                  fontSize: 9,
                  fontWeight: FontWeight.w700,
                  color:
                      fc.flat ? Colors.white : (fc.momPct! > 0 ? _bad : _good)),
            ),
          ),
        ],
        const SizedBox(height: 6),
        SizedBox(
          ...
          child: Stack(children: [
            Positioned.fill(
              child: CustomPaint(
                painter: _ForecastPainter(
                    ...
                    daysInMonth: data.daysInMonth.toDouble(),
                    projTotal: fc.projected),
              ),
            ),
            Positioned(
              left: (fc.chipLeftFrac * (MediaQuery.of(context).size.width - 76))
                  .clamp(0, 160)
                  .toDouble(),
              top: 4,
            ...
            Positioned(
                right: 10,
                top: 16,
                child: _chip('Projected', rm(fc.projected), light: false)),
          ]),
        ),
      ]),
    );
  ...
  Widget _chip(String label, String value, {required bool light}) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border:
            light ? Border.all(color: Colors.white.withOpacity(0.22)) : null,
        boxShadow: light
            ? null
            : [
                BoxShadow(
          ...
          children: [
            Text(label,
                style: TextStyle(
                    ...
                    color: light
                        ? const Color(0xFFA9C7F5)
                        : const Color(0xFF6B7280))),
            Text(value,
                style: TextStyle(
                    fontSize: 13,
                    fontWeight: FontWeight.w800,
                    color: light ? Colors.white : const Color(0xFF0A1E4F))),
          ]),
    );
  }
                    ...
                    child: Padding(
                      ...
                      child: Align(
                        ...
                        child: FractionallySizedBox(
                          heightFactor:
                              (bars[i].value / barMax).clamp(0.018, 1.0),
                          child: Container(
                            decoration: BoxDecoration(
                              color: bars[i].future
  ...
  Widget _buildStatGrid(
      _MonthData data, Map<String, List<AppCategory>> allCats) {
  ...
  Widget _paceCard(_MonthData data) {
    if (data.pacePct == null) {
      return _statCard(
          'SPENDING PACE', '—', 'Need 8+ days of data this month.');
    }
    final up = data.pacePct! >= 0;
    final ink = up ? _bad : _good;
    return _card(
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const Text('SPENDING PACE',
          style: TextStyle(
              fontSize: 9,
              fontWeight: FontWeight.w700,
              letterSpacing: 1.1,
      ...
      Row(children: [
        ...
        Text('${up ? '+' : '−'}${data.pacePct!.abs().toStringAsFixed(0)}%',
            style: TextStyle(
                fontSize: 22,
                fontWeight: FontWeight.w800,
                color: ink,
      ...
      Expanded(
          child: Text(
              '${rm(data.paceNow!)}/day now vs ${rm(data.paceBefore!)}/day before',
              style: const TextStyle(
                  fontSize: 10,
                  fontWeight: FontWeight.w600,
                  color: AppTheme.onSurfaceVariant,
  ...
  Widget _topCategoryCard(
    ...
    return _card(
      ...
      const SizedBox(height: 4),
      Row(children: [
        Text(categoryEmoji(allCats, top.label),
      ...
      const SizedBox(height: 4),
      Expanded(
          child: Text('${rm(top.value)} · ${top.pct.round()}% of total spend',
              style: const TextStyle(
                  fontSize: 10,
                  fontWeight: FontWeight.w600,
                  color: AppTheme.onSurfaceVariant,
    ...
    return _card(
      ...
      Row(
          ...
          children: [
            ...
            const Spacer(),
            Text(
                '${data.momPct! >= 0 ? '+' : '−'}${data.momPct!.abs().toStringAsFixed(1)}%',
                style: TextStyle(
                    fontSize: 18,
                    fontWeight: FontWeight.w800,
                    color: data.momPct! >= 0 ? _bad : _good)),
          ]),
      const SizedBox(height: 6),
      Text(
          '${rm(data.spent)} by day ${data.daysElapsed} against ${rm(data.prevAtSamePoint!)} at the same point in ${kMonthNames[data.momMonths[1].month.month - 1]}.',
          style: const TextStyle(
              fontSize: 11, color: AppTheme.onSurfaceVariant, height: 1.5)),
      const SizedBox(height: 16),
      ...
      for (final m in data.momMonths)
        Padding(
          padding: const EdgeInsets.only(bottom: 11),
          child: Row(children: [
            SizedBox(
                width: 30,
                child: Text(kMonthNames[m.month.month - 1].toUpperCase(),
                    style: TextStyle(
                        fontSize: 9,
                        fontWeight: FontWeight.w700,
                        letterSpacing: 0.6,
            ...
            Expanded(
              child: SizedBox(
                  ...
                  child: Row(children: [
                    Expanded(
                        flex: max((m.recorded / maxTotal * 1000).round(), 1),
                        child: Container(
                            decoration: BoxDecoration(
                                color: m.isCurrent
                                    ? AppTheme.secondary
                    ...
                    if (m.projectedExtra > 0)
                      Expanded(
                          flex: max(
                              (m.projectedExtra / maxTotal * 1000).round(), 1),
                          child: Container(
                              margin: const EdgeInsets.only(left: 2),
                              decoration: BoxDecoration(
                                  color: const Color(0xFF93C5FD),
                                  borderRadius: BorderRadius.circular(4)))),
                    if (m.recorded + m.projectedExtra < maxTotal)
                      Expanded(
                          flex: max(
                              ((maxTotal - m.recorded - m.projectedExtra) /
                                      maxTotal *
                                      1000)
                                  .round(),
                              0),
      ...
      Padding(
        ...
        child: Container(
          ...
          decoration: const BoxDecoration(
              border: Border(top: BorderSide(color: Color(0xFFEDEFF6)))),
          child: Text(
            ...
            style: const TextStyle(
                fontSize: 10, color: AppTheme.onSurfaceVariant, height: 1.5),
          ),
        ),
      ),
    ]));
      ...
      SizedBox(
        ...
        child: PageView.builder(
          ...
          itemBuilder: (context, i) =>
              _spikeAlertCard(spikes[i], data, allCats),
        ),
      ),
      if (spikes.length > 1) ...[
          ...
          children: [
            Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Container(
                ...
                height: 40,
                decoration: BoxDecoration(
                    color: _bad, borderRadius: BorderRadius.circular(13)),
              ...
              Expanded(
                child: Column(
                    ...
                    children: [
                      ...
                      Text(
                          '${_dowLabel(s.date)}, ${s.date.day} ${kMonthNames[s.date.month - 1]}',
                          style: const TextStyle(
                              fontSize: 16,
                              fontWeight: FontWeight.w800,
                              color: Color(0xFF1A1C2E)),
                      ...
                      Container(
                        ...
                        child: Row(mainAxisSize: MainAxisSize.min, children: [
                          Text(categoryEmoji(allCats, s.topCategory),
                              style: const TextStyle(fontSize: 12)),
                          const SizedBox(width: 5),
                          Flexible(
                              child: Text(
                                  'Mostly ${s.topCategory.toLowerCase()}',
                                  style: const TextStyle(
                                      fontSize: 11,
                                      fontWeight: FontWeight.w700,
                                      color: Color(0xFF1A1C2E)),
              ...
              Column(
                  ...
                  children: [
                    ...
                    Text(
                        '${multiplier.toStringAsFixed(1)}× above\nyour average',
                        textAlign: TextAlign.right,
                        style: TextStyle(
                            fontSize: 9.5,
                            fontWeight: FontWeight.w700,
      ...
      for (var i = 0; i < data.smartInsights.length; i++)
        Padding(
          padding: EdgeInsets.only(
              bottom: i == data.smartInsights.length - 1 ? 0 : 10),
  ...
  Widget _insightTile(_Insight insight) {
    final bg =
        insight.alert ? const Color(0xFFFFF3F2) : const Color(0xFFF3F8FE);
...
class _MonthData {
  ...
  final double? momPct;
  final List<_MomMonth>
      momMonths; // [this month, prev, prev-1, ...] up to 6, empty if <2
  ...
  const _MonthData({
    ...
    this.paceBefore,
    this.prevAvgDaySub,
    this.prevAtSamePoint,
    this.momPct,
    required this.momMonths,
  ...
  List<_Bar> dailyBars() {
    return [
      for (var d = 1; d <= daysInMonth; d++)
        _Bar(
          label:
              '${_dowLabel(DateTime(month.year, month.month, d))}, $d ${kMonthNames[month.month - 1]}',
  ...
  static double _monthTotalToDay(
      List<tx.Transaction> all, DateTime monthStart, int upToDay) {
    return all
        .where((t) =>
            isAnomalyEligibleExpense(t, monthStart.year, monthStart.month) &&
            t.date.day <= upToDay)
  ...
  static _MonthData compute(List<tx.Transaction> all, DateTime month) {
    ...
    final daysElapsed =
        isCurrentMonth ? min(now.day, daysInMonth) : daysInMonth;
    ...
    final prevAtSamePoint = _monthHasData(all, prevMonthStart)
        ? _monthTotalToDay(all, prevMonthStart, daysElapsed)
        : null;
    ...
    if (sortedCats.isNotEmpty) {
      topCategory = _TopCategory(
        label: sortedCats.first.key,
        value: sortedCats.first.value,
        pct: sortedCats.first.value / spent * 100,
        color: kCategoryPalette[0],
      );
    }

    // Unusual spending days: IQR rule shared with the notification detector.
    final anomalyThreshold = computeDailySpendingThreshold(recorded);
    final spikes = detectUnusualSpendingDays(
            dailyTotals: recorded, monthTx: monthTx, month: month)
        .map((d) =>
            _Spike(date: d.date, amount: d.amount, topCategory: d.topCategory))
        .toList();
    ...
    if (pacePct != null) {
      ...
      insights.add(_Insight(
        ...
        alert: up,
        text:
            'Your spending over the last 7 days is ${pacePct.abs().toStringAsFixed(0)}% ${up ? 'higher' : 'lower'} than earlier this month.',
    ...
    if (forecast != null) {
      if (forecast.prevMonthTotal != null) {
        ...
        insights.add(_Insight(
          ...
          alert: false,
          text:
              'At the current pace the month ends near ${rm(forecast.projected)}.',
          rule: 'FORECAST FROM $daysElapsed DAYS OF TRANSACTIONS',
        ));
      }
    ...
    return _MonthData(
      ...
      prevAvgDaySub: prevAvgDaySub,
      prevAtSamePoint: prevAtSamePoint,
      momPct: momPct,
      momMonths: momMonths,
      spikes: spikes,
...
class _ForecastPainter extends CustomPainter {
  ...
  const _ForecastPainter(
      {required this.cumulative,
      required this.daysInMonth,
      required this.projTotal});

  @override
  void paint(Canvas canvas, Size size) {
    if (projTotal <= 0 || cumulative.length < 2) return;
    const topMargin = 8.0, bottomMargin = 4.0;
    double px(double day) =>
        (day / daysInMonth * size.width).clamp(0, size.width);
    double py(double v) =>
        size.height -
        bottomMargin -
        (v / projTotal).clamp(0, 1) * (size.height - topMargin - bottomMargin);

    final points = <Offset>[
      for (var i = 0; i < cumulative.length; i++)
        Offset(px(i.toDouble()), py(cumulative[i]))
    ];
    ...
    canvas.drawPath(
        ...
        Paint()
          ..shader = LinearGradient(
              ...
              colors: [
                const Color(0xFF2E7FD4).withOpacity(0.55),
                const Color(0xFF2E7FD4).withOpacity(0)
...
class _DashedHLine extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    while (x < size.width) {
      canvas.drawLine(
          Offset(x, 0), Offset(min(x + dashLen, size.width), 0), paint);
      x += dashLen + gapLen;
    }
  }
